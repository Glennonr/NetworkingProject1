...
import time
"""
This program runs a chat client using UDP Protocol
"""
PORT = 23457
...
class ChatProtocol(asyncio.DatagramProtocol):
    ...
    async def send_messages(self):
        """
        Prompts the peer for input for usernme and then message
        ...
        while True:
            ...
            self.transport.sendto(('3' + str(time.strftime('%X')) + ' ' + self.username + ' ' + message).encode(), ('255.255.255.255', PORT))
        ...
        if addr[0] != get_ip():            
            if data[0] == '1':
                ...
                if username == self.username:
                    ...
                    self.transport.sendto(('True').encode(), addr)
                    for message in self.messages[-10:]:
                        self.transport.sendto(('3' + message).encode(), addr)

            if data == 'False': 
                self.transport.close()

            if data == 'True':
                self.transport.sendto(('2').encode(), addr)

        if data[0] == '3' and data[1:] not in self.messages:
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Chat Room')
    parser.add_argument('--port', type=int, help='The port to bind to', default=27999)
